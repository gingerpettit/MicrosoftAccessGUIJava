...
public class tableUpdateForm extends JFrame{
	tableUpdatePanelBuilder 		tableInfoPanel;		// panel for user information
	...
	public tableUpdateForm() throws SQLException{
		...
		tableInfoPanel = new tableUpdatePanelBuilder();
		...
		add(tableInfoPanel, BorderLayout.NORTH);
	...
	private void buildListPanel() throws SQLException {
		...
		listPanel.setBorder(BorderFactory.createTitledBorder("Table Information"));
		
		// create an object to access the db
		tableTableManager um = new tableTableManager();
		
		// create a result set to hold the blank search for when first startup
		ResultSet tableInfo = um.selectTables("");
		
		// user rs2xml to display our resultset
		ourTable = new JTable(DbUtils.resultSetToTableModel(tableInfo));
				...
				public void valueChanged(ListSelectionEvent event) {
					...
					if (ourTable.getSelectedRow() >= 0) {
						...
						try {
							...
							ResultSet updateInfo = tableTableManager.selectUpdate(selectedRecordID);

							while(updateInfo.next()) {
								tableInfoPanel.setName(updateInfo.getString("name"));
							}
...
private void resetForm() throws SQLException {
		...
		searchInfo = tableTableManager.selectTables("");
...
public class SearchButtonListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e1) {
		...
		String strName = tableInfoPanel.getName();
		...
		try {
			...
			searchInfo = um.selectTables(strName);
...
public class InsertButtonListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		String name = tableInfoPanel.getName();
		...
		try {
			...
			tableTableManager tableManager = new tableTableManager();
			
			// insert the record
			rows = tableManager.insertRecord(name);
...
public class UpdateButtonListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e1) {
		...
		int rows = 0;
		tableTableManager updater;
		
		
		try {
			// create an object to instantiate the Connection to the table	
			updater = new tableTableManager();
...
public class DeleteButtonListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e1) {
		...
		int rows = 0;
		tableTableManager Deleter;
		
		
		try {
			// create an object to instantiate the Connection to the table	
			Deleter = new tableTableManager();
...
public class ExitButtonListener implements ActionListener {
	...
	tableUpdateForm employee = new tableUpdateForm();
	}
